...
#aditmadzs = LineClient(auth='EJn1NyzuRubhVidJIox6.GaMVDmEF24EJLyWZB69bXG.bHNcPQWhdsisqQDPWsR2hDtNanfbDJxKf9cZ+BxdBfE=')
aditmadzs.log("Auth Token : " + str(aditmadzs.authToken))
channel = LineChannel(aditmadzs)
aditmadzs.log("Channel Access  : " + str(channel.channelAccessToken))

#ki = LineClient()
#ki = LineClient(auth='LOGIN ')
#ki.log("Auth  : " + str(ki.authToken))
#channel1 = LineChannel(ki)
#ki.log("Channel Access  : " + str(channel1.channelAccess))

#ubah mid di dalem admin,owner,creator.json dengan mid kalian
poll = LinePoll(aditmadzs)
#call = aditmadzs
creator = ["u8009af74c79fdb87a3958c336faf41c6"]
owner = ["uf21efd5addc2bac1922bca90d7e6e1f5"]
admin = ["uf21efd5addc2bac1922bca90d7e6e1f5"]
staff = ["uf21efd5addc2bac1922bca90d7e6e1f5"]
...
def welcomeMembers(to, mid):
    try:
        arrData = ""
        textx = "BlackChing menyabut Anda\nSalken  ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
...
def leaveMembers(to, mid):
    try:
        arrData = ""
        textx = "Kok Left sih kk...Nih poto mu ambil aja ku gk butuh\nSana lu balik ke nenek moyang lu  ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
            ...
            arrData = {'S':slen, 'E':elen, 'M':i}
            arr.append(arrData)
            textx += mention+wait["leave"]+"\n jangan balik lg dari group micin "+str(ginfo.name)
            if no < len(mid):
                no += 1
                textx += "%i " % (num)
                num=(num+1)
...
def help():
    ...
    helpMessage = "            " + "\n" + \
                  ...
                  "╠🐾❮ " + key + "Listadmin\n" + \
                  "╠🐾❮ " + key + "Listprotect\n" + \
                  "╠🐾❮ Ketik「 Refresh 」Jika Sudah\n╠🐾❮ Menggunakan Cmd Diatas\n" + \
                  "╠════════════════════╗" + "\n" + \
                  "║               ]  𝗕𝗹𝗮𝗰𝗸❮𝗖𝗵𝗶𝗻𝗴  [             ║" + "\n" + \
                  "╠════════════════════╝" + "\n" + \
                  "╠══════════════════════╗" + "\n" + \
                  "║https://line.me/ti/g2/JBKAV3NCA6 ║" + "\n" + \
                  "╚══════════════════════╝"
    return helpMessage
...
#===========BOT UPDATE============#
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                              if msg.toType == 2:
                                ...
                                aditmadzs.sendMessage(msg.to,"Kirim foto mantan kak.....")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                aditmadzs.sendMessage(msg.to,"Kirim fotonya bugilnya.....")
...
#===========BOT UPDATE============#
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                aditmadzs.sendMessage(msg.to,"⏩ Admin DandyHayate\n\n⏩Creator BOT:\n"+ma+"\n⏩Admin:\n"+mb+"\n⏩Staff:\n"+mc+"\n⏩Total「%s」" %(str(len(owner)+len(admin)+len(staff))))
